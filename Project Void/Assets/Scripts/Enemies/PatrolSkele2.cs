...
public class PatrolSkele2 : MonoBehaviour
{
    Animator anim;
    ...
    private void Start()
    {
        ...
        isDead = sceneInfo.skele2Dead;

        transform.position = sceneInfo.skele2Point;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (!isDead)
        {
            if (other.tag == "Player")
            {
                ...
                sceneInfo.skele2Point = transform.position;
                SceneManager.LoadScene("Battle2Skele");
            }
        }
    }
